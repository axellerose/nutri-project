...
const postCreateRecipe = (req, res, next) => {
  ...
  req.body.productIds.forEach((elem, idx) =>
    newRecipe.products.push({
      product: elem,
      quantity: req.body.quantities[idx],
    })
  );
...
const getRecipeDetails = (req, res, next) => {
  ...
  Recipe.findById(req.params.recipeId)
    ...
    .then((recipe) => {
      const calculateProp = (products, prop) =>
        products.reduce(
          (acc, curr) => acc + curr.product.info[prop] * (curr.quantity / 100),
          0
        );
      const calculateTotal = (prop, quantity) =>
        ((prop * 100) / quantity).toFixed(2);

      function calculateRecipe(recipe) {
        const totalQty = recipe.products.reduce(
